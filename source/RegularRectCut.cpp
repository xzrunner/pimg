...
RegularRectCut::RegularRectCut(const uint8_t* pixels, int w, int h, const CU_VEC<CRect>& pre_rects)
{
	LoadPixels(pixels, w, h);
	...
	for (int i = 0, n = pre_rects.size(); i < n; ++i) {
		const CRect& r = pre_rects[i];
		m_area_array->CutByCRect(r.x, r.y, r.w, r.h, m_left_area);
...
// 	while (m_left_area > 0) {
// 		int x, y;
// 		int area = CalBestCRectPos(TRY_MIN_EDGE, TRY_MIN_EDGE, x, y);
...
void RegularRectCut::AutoCutWithLimit(float limit)
{
	...
	for (int i = 0; i < EDGE_COUNT && m_left_area > 0; ++i)
	{
		...
		for (int j = 0; j < 3; ++j)
		{
			...
			do {
				int x, y;
				int area = CalBestCRectPos(w, h, x, y);
				...
				if (success) {
					if (j == 0) {
						if (m_area_array->GetCRectArea(x, y, hw, hh) < area_limit_min ||
							...
							m_area_array->GetCRectArea(x+hw, y+hh, hw, hh) < area_limit_min ||
							m_area_array->GetCRectArea(x+hw, y, hw, hh) < area_limit_min) {
							success = false;
						}
					} else if (j == 1) {
						if (m_area_array->GetCRectArea(x, y, hw, hh) < area_limit_min ||
...
int RegularRectCut::CalBestCRectPos(int w, int h, int& ret_x, int& ret_y)
{
	int max_area = -1;
	int max_x, max_y;
	for (int y = 0, up = m_height - h; y <= up; y+=1) {
		for (int x = 0, right = m_width - w; x <= right; x+=1) {
			int area = m_area_array->GetCRectArea(x, y, w, h);
			if (area > max_area) {
				max_area = area;
				max_x = x;
				max_y = y;
