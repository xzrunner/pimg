...
OutlineFine::OutlineFine(const CU_VEC<sm::vec2>& raw_border,
...
void OutlineFine::ReduceOutlineCount(float area_tol, float perimeter_tol)
{
										  ...
										  int max_step, bool reduce_count)
{
	...
	sm::vec2 bound[4];
	sm::MinBoundingBox::Do(m_raw_border, bound);
	...
	do {
		...
		float area = sm::get_polygon_area(m_fine_border);
...
void OutlineFine::RemoveOneNode(int idx, sm::vec2& new0, sm::vec2& new1, float& decrease) const
{
	decrease = 0;
	...
	for (float i = 0; i < len_prev; i+=1) {
		...
		for (float j = idx_e; j < len_next; j+=1) {
			...
			if (IsCutTriLegal(curr, s, e)) {
				float a = sm::get_triangle_area(s, curr, e);
				if (a > decrease) {
					decrease = a;
					new0 = s;
					new1 = e;
...
bool OutlineFine::IsAddTriLeagal(const sm::vec2& p0, const sm::vec2& p1, const sm::vec2& p2) const
{
	...
	for (int i = 0; i < STEPS_COUNT; ++i) {
		...
		while (true)
		{
			...
			for (int j = 0, m = m_rotate_lut.size(); j < m; ++j)
			{
				...
				float area = sm::get_triangle_area(start, curr_mid, end);
				if (area > score &&
...
void OutlineFine::ReduceNode(float tolerance)
{
	if (m_fine_border.size() <= 3) {
		return;
	}

	float area_limit = sm::get_polygon_area(m_fine_border) * tolerance;
...
void OutlineFine::ReduceEdge(float area_tol, float perimeter_tol)
{
	if (m_fine_border.size() <= 3) {
		return;
	}

	float area_limit = sm::get_polygon_area(m_fine_border) * area_tol;
	float perimeter_limit = sm::get_polygon_perimeter(m_fine_border) * perimeter_tol;
	bool success = false;
	do {
		ReduceNode(area_tol);
		if (m_fine_border.size() <= 3) {
...
// 				}
				...
				if (is_inside || sm::get_triangle_area(intersect, start, end) < area_limit) {
...
void OutlineFine::InitRotateLUT()
{
