...
RegularRectCondense::RegularRectCondense(const CU_VEC<CRect>& rects, int width, int height, bool* ori_pixels)
	: m_width(width)
	, m_height(height)
{
...
void RegularRectCondense::Condense(const CRect& r)
{
...
void RegularRectCondense::Insert(const CRect& r)
{
	Grid* g = new Grid(r);
	m_grids.push_back(g);
	InsertPixelGrid(r, g);
...
void RegularRectCondense::GetSortedCRects(CU_VEC<CRect>& rects) const
{
	rects.clear();
...
void RegularRectCondense::CondenseCovered(const CRect& r)
{
	CU_VEC<Grid*> grids;
	for (int i = 0, n = m_grids.size(); i < n; ++i) {
		if (IsCRectIntersect(r, m_grids[i]->r)) {
...
void RegularRectCondense::RemovePixelGrid(const CRect& r, Grid* g)
{
	for (int x = r.x; x < r.x + r.w; ++x) {
		for (int y = r.y; y < r.y + r.h; ++y) {
			if (x >= 0 && x < m_width && y >= 0 && y < m_height) {
...
void RegularRectCondense::InsertPixelGrid(const CRect& r, Grid* g)
{
	for (int x = r.x; x < r.x + r.w; ++x) {
		for (int y = r.y; y < r.y + r.h; ++y) {
			if (x >= 0 && x < m_width && y >= 0 && y < m_height) {
