...
OutlineRaw::OutlineRaw(const uint8_t* pixels, int w, int h)
	: m_pixels(pixels)
	, m_width(w)
	, m_height(h)
{
...
void OutlineRaw::CreateBorderPoints()
{
...
void OutlineRaw::CreateBorderConvexHull()
{
	sm::convex_hull(m_border_line, m_convex_hull);
}

void OutlineRaw::CreateBorderLine()
{
	m_border_line.clear();
...
void OutlineRaw::MergeBorderLine()
{
...
bool OutlineRaw::IsPixelBorder(int x, int y) const
{
	if (x < 0 || x >= m_width ||
		y < 0 || y >= m_height) {
		return false;
...
bool OutlineRaw::IsPixelTransparente(int x, int y) const
{
	if (x < 0 || x >= m_width ||
		y < 0 || y >= m_height) {
		return true;
...
bool OutlineRaw::IsPixelTransparente(bool* flag, int x, int y) const
{
	if (x < 0 || x >= m_width ||
		y < 0 || y >= m_height) {
		return true;
...
bool OutlineRaw::IsNearby(const sm::vec2& p0, const sm::vec2& p1) const
{
	return abs(p0.x - p1.x) <= 1 && abs(p0.y - p1.y) <= 1;
}

void OutlineRaw::MergeLine(const CU_VEC<sm::vec2>& src, CU_VEC<sm::vec2>& dst)
{
	if (src.size() < 3) {
		return;
	}
	...
	for (int i = 2, n = src.size(); i < n; ++i)
	{
		...
		if (sm::is_two_line_parallel(dst[dst.size()-1], dst[dst.size()-2], dst[dst.size()-1], p)) {
			dst.pop_back();
		}
		dst.push_back(p);
	}
